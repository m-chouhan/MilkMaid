 ...
 */
 ...
 public class StrongerPainter extends Painter {
    ...
    public StrongerPainter(VertexQueue VQ,Vector2 node,GameSuperviser superviser) {
        ...
        //VQueue = VQ;
        PlayerNode = node;
        PlayerSprite = new Sprite(Regions[0][3]);
    ...
    public void render(float v) {
        ...
        for(int i = 0;i<VQueue.getSize();++i) {
            ...
            if(vertex.getCurrentState() == Vertex.Status.Invisible ||
                    vertex.getCurrentState() == Vertex.Status.Dead ) continue;
        ...
        PlayerSprite.setPosition(PlayerNode.x - Regions[0][3].getRegionWidth()/2,
                PlayerNode.y-Regions[0][3].getRegionHeight()/2);
